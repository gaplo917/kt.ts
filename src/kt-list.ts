...
export interface KtListOp<T>
  ...
  //
  // ktForEachIndexed(action: (value: T, index: number) => void): void
  //
  // max(): number | null
  //
...
export function injectAllKtProto(proto: any) {
  injectDistinct(proto)
  injectFilter(proto)
  injectForEach(proto)
  injectSize(proto)
  injectContains(proto)
  injectElementAt(proto)
  injectFind(proto)
  injectFirst(proto)

  // extras
  injectFilterExtra(proto)
}

;(() => {
  injectAllKtProto(Array.prototype)
})()
